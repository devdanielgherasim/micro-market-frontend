        ...
        <div className={`space-y-3 sm:space-y-4 ${className}`}>
            ...
            {loading && !orders.length ? (
                <div className="flex justify-center items-center py-6 sm:py-8 md:py-10">
                    <div
                        className="animate-spin rounded-full h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 border-2 border-green-500 dark:border-green-400 border-t-transparent shadow-md"></div>
                </div>
            ) : orders.length > 0 ? (
                <>
                    <div
                        className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
                        {orders.map((order) => (
                            <OrderCard
                                key={order.id}
                                order={order}
                    ...
                    {pagination && pagination.totalPages > 1 && (
                        <div className="flex justify-center items-center mt-6 space-x-2">
                            <Button
                                ...
                                className={`${pagination.first ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                            >
                ...
                Page {pagination.page + 1} of {pagination.totalPages}
                            ...
                            <Button
                                ...
                                onClick={() => handlePageChange(currentPage + 1)}
                                className={`${pagination.last ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                                          d="M9 5l7 7-7 7"/>
                ...
                <div
                    ...
                    <svg className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-3 text-gray-400 dark:text-gray-500" fill="none"
