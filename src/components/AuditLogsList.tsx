...
import { AuditLogEntry, AuditLogFilter, AuditActionType, AuditResourceType } from '../types/audit';
...
export const AuditLogsList: React.FC<AuditLogsListProps> = ({ initialFilter }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Filter Controls */}
      <div className="bg-white dark:bg-secondary-800 shadow-sm rounded-lg border border-secondary-200 dark:border-secondary-700 overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Date Range */}
          <div>
            <label htmlFor="startDate" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-1">Start Date</label>
            <input
              type="date"
              id="startDate"
          ...
          <div>
            <label htmlFor="endDate" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-1">End Date</label>
            <input
              type="date"
              id="endDate"
          ...
          {/* Action Type */}
          <div>
            <label htmlFor="action" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-1">Action Type</label>
            <select
              id="action"
              value={filter.action || ''}
          ...
          {/* Resource Type */}
          <div>
            <label htmlFor="resourceType" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-1">Resource Type</label>
            <select
              id="resourceType"
          ...
          {/* Resource ID */}
          <div>
            <label htmlFor="resourceId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-1">Resource ID</label>
            <input
              type="text"
              id="resourceId"
          ...
          {/* User ID */}
          <div>
            <label htmlFor="userId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-1">User ID</label>
            <input
              type="text"
              id="userId"
        ...
        <div className="px-4 py-3 bg-secondary-50 dark:bg-secondary-900 text-right flex justify-end space-x-3 border-t border-secondary-200 dark:border-secondary-700">
          <button
      ...
      {loading && !auditLogs.length && (
        <div className="flex justify-center items-center py-6 sm:py-8 md:py-10">
          <div className="animate-spin rounded-full h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 border-2 border-primary-500 dark:border-primary-400 border-t-transparent shadow-md"></div>
        </div>
      )}

      {/* Data Table */}
      {!loading && auditLogs.length === 0 ? (
        <div className="text-center py-6 sm:py-8 md:py-10 text-sm sm:text-base text-secondary-500 dark:text-secondary-400 bg-secondary-50 dark:bg-secondary-800/50 rounded-lg shadow-sm p-4 sm:p-6">
        ...
        <div className="bg-white dark:bg-secondary-800 shadow-sm rounded-lg border border-secondary-200 dark:border-secondary-700 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-secondary-200 dark:divide-secondary-700">
              <thead className="bg-secondary-50 dark:bg-secondary-900/50">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">Time</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">User</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">Action</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">Resource</th>
          ...
          {/* Pagination */}
          <div className="px-4 py-3 flex items-center justify-between border-t border-secondary-200 dark:border-secondary-700 sm:px-6">
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-secondary-700 dark:text-secondary-300">
                  Showing <span className="font-medium">{auditLogs.length}</span> results of <span className="font-medium">{pagination.total}</span> total
                </p>
              </div>
              <div>
                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  ...
                  <div className="relative inline-flex items-center px-4 py-2 border border-secondary-300 dark:border-secondary-700 bg-white dark:bg-secondary-800 text-sm font-medium text-secondary-700 dark:text-secondary-300">
                    Page {filter.page || 1}
                  </div>
                  <button
                    onClick={nextPage}
