...
const NavItem: React.FC<NavItemProps> = ({href, icon, label, isActive = false, onClick}) => {
    const baseClasses = "group flex items-center w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-medium rounded-lg transition-all duration-200";
      ...
      <span
        ...
        {icon}
          <span className="ml-2 sm:ml-3">{label}</span>
            ...
            {isActive && (
                <span className="ml-auto h-1.5 sm:h-2 w-1.5 sm:w-2 rounded-full bg-primary-500 dark:bg-primary-400"></span>
            )}
        </Link>
    );
};
    ...
    return (
        <nav className="flex flex-col space-y-1.5 w-full">
            ...
            {isAuthenticated ? (
                <button
                    onClick={() => logout()}
                    className="group flex items-center w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-medium rounded-lg transition-all duration-200 text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-800/70 hover:text-danger-600 dark:hover:text-danger-400"
                >
          <span
            ...
            <span className="ml-2 sm:ml-3">Logout</span>
          </span>
                </button>
            ) : (
                <GuestOnly>
                    <button
                        onClick={() => login()}
                        className="group flex items-center w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-medium rounded-lg transition-all duration-200 text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-800/70 hover:text-primary-600 dark:hover:text-primary-400"
                    >
            <span
              ...
              <span className="ml-2 sm:ml-3">Login</span>
            </span>
                    </button>
                </GuestOnly>
            )}
